CREATE OR REPLACE FUNCTION nhap_diem_cho_sinh_vien(
    p_ma_sinh_vien VARCHAR,
    p_ma_lop_hoc_phan INT,
    p_diem_qua_trinh NUMERIC(4,2),
    p_diem_cuoi_ky NUMERIC(4,2)
) RETURNS TEXT AS $$
DECLARE
...
BEGIN
    ...
    RETURN FORMAT(
        ' Đã cập nhật điểm cho sinh viên % trong lớp học phần %: QT = %.2f, CK = %.2f, Kết thúc = %.2f',
        p_ma_sinh_vien, p_ma_lop_hoc_phan, p_diem_qua_trinh, p_diem_cuoi_ky, v_diem_ket_thuc
    );

EXCEPTION
    WHEN OTHERS THEN
        RETURN ' Lỗi khi nhập điểm: ' || SQLERRM;
END;
$$ LANGUAGE plpgsql;
